</div>
</div>
<div class="footer">
	<div class="container wise">
		<div class="text">Zeit für Leicht-Sinn im Freiraum</div>
	</div>
	<div class="baseline">
		<div class="container nav">
			<a href="https://www.facebook.com/freiRaumAbtenau/" target="_blank"><span class="socialicon icon--facebook"></span></a>
			<a href="">impressum</a>
			<a href="contact" class='js-navigation-link js-footer-link' data-page="contact">kontakt</a>
			<a href="partner" class="js-navigation-link js-footer-link" data-page="partner">partner</a>
		</div>
	</div>
</div>
<script src="//ajax.googleapis.com/ajax/libs/jquery/1.12.2/jquery.min.js"></script>
<script>window.jQuery || document.write("<script src='<?php echo get_template_directory_uri(); ?>/assets/web/js/jquery.min.js'>\x3C/script>")</script>
<script type='text/javascript' src='<?php echo get_template_directory_uri(); ?>/assets/js/vendor.min.js'></script>
<script type='text/javascript' src='<?php echo get_template_directory_uri(); ?>/assets/js/scripts.min.js'></script>
<?php wp_footer(); // https://codex.wordpress.org/Plugin_API/Action_Reference/wp_footer ?>
</body>
</html>